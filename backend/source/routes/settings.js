...
settings_router.post("/upload-profile", protect, upload.single("image"), async(req, res) => {
    try {
...
settings_router.get("/view-profile", protect, (req, res) => {
    res.json(req.user); // This will return the authenticated user data
});

settings_router.post("/upload-photos", protect, upload.array("images", 5), async(req, res) => {
    try {
        const user = await User.findById(req.user._id);
